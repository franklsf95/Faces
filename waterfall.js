	...
	$.fn.waterfall = function(options){
		...
		function initParam(){
			...
			ablock_css = {'width':settings.block_width,
				'margin':settings.margin,'padding-top':15};
			$("."+settings.block_class).css(ablock_css);
			start = 0, block_height = [], column_height=[], new_block = 
			"<div class='ablock'>"+
				"<a target='_blank' href='#'>"+
					"<img class='pic' src='{0}' width='"+(settings.block_width-30)+"px'>"+
					"<div style='width: "+(settings.block_width-60)+"px;padding-left:15px' class='names'>"+
						"<a class='name'>{3}</a>"+
						"<a class='school_name'>{2}</a>"+
					"</div>"+
				"</a>"+
					"<div style='width: 30px;padding-right:15px;' class='votes'>"+
                    	"<a class='score' id=\"scores{1}\" vote=\"{4}\">{4}</a>"+
                	"</div>"+
				
				"<div class='rating' style='width:"+(settings.block_width)+"px' id='vote'>"+
					"<a href=\"javascript:;\" class='like'  pid=\"{1}\" onclick=\"var pid = $(this).attr('pid');$(this).parent().removeClass('rating').addClass('rated');$(this).html('已喜欢！');var vote=$('#scores'+pid).attr('vote');var number = parseInt(vote)+1;$(this).parent().parent().children('.votes').children('#scores'+pid).html(number);$.get('/index.php/face/vote.shtml',{pid: pid},function(data,status){});return false;\">我喜欢！</a>"+
				"</div>"+
			"</div>"+
				...
				success: function(data){
					...
					$.each(data, function(i) {
						imgReady(data[i].src, function() {
							$element.append(new_block.template(data[i].src,data[i].id,data[i].college,data[i].name,data[i].likes));
